...
with c.cursor() as cur:
    cur.execute("""
        SELECT id, heart_rate, heart_rate_samples_count,
               CHAR_LENGTH(heart_rate_samples) AS samples_chars,
               SUBSTRING(heart_rate_samples, 1, 100) AS preview,
               created_at
        FROM watch_records WHERE user_id=39 ORDER BY id DESC LIMIT 5
    """)
    print("id | hr | samples_count | chars | preview | at")
